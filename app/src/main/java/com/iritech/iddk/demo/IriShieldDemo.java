...
{	
	...
	{ 
        ...
        if(isShowImages == false) showDialog("Warning", "Streamming function is disabled in the device !");
		...
		else
		{
			...
			{	
				...
				if(IRIS_ENROLL == irisRegCurrentAction)
				{
					...
					{
						mIsBadQualityImage = true;
						if(numAcceptableEyes == 1 && !isGrayZone) infor += "Do you want to proceed anyway?\n";
			...
			if(mCurrentStatus.getValue() == IddkCaptureStatus.IDDK_COMPLETE)
			{
				//Get the best image
		 			...
		 			(isBinocularDevice && (iRet.intValue() == IddkResult.IDDK_OK || iRet.intValue() == IddkResult.IDDK_SE_LEFT_FRAME_UNQUALIFIED || iRet.intValue() == IddkResult.IDDK_SE_RIGHT_FRAME_UNQUALIFIED)))
		 		{
					...
					{
						if(mManiaConfig.th_qmscore_show) updateCurrentStatus("Total Score = " + quality.get(0).getTotalScore() + ", Usable Area = " + quality.get(0).getUsableArea());
					}
